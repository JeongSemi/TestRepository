...
--bno->번호, btitle->제목, bwriter->글쓴이 로 컬럼 이름을 변경해서 가져오시오 (as는 생략가능)
...
--정렬 후, 행 번호 매기기
select rownum, bno, btitle, bwriter, bdate, bhitcount
from (select bno, btitle, bwriter, bdate, bhitcount from board order by bno desc); --서브쿼리

--특정 행 번호 이하만 가져오기(Top N을 가져오기)
select rownum, bno, btitle, bwriter, bdate, bhitcount
from (select bno, btitle, bwriter, bdate, bhitcount from board order by bno desc) --서브쿼리
where rownum<=10;
